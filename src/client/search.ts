 ...
 */
export function dmsFold(text: string): string {
  return text.normalize('NFKD').replace(/\p{M}+/gu, '').toLowerCase()
}

/** 搜索词归一化（去首尾空白后折叠）。空串 = 非搜索态。 */
export function dmsNormalizeQuery(raw: string): string {
  return dmsFold(raw.trim())
}

/** 模型条目的搜索文本（haystack）。与 choices 构建处同源，避免两处漂移。 */
export function dmsHaystack(group: ModelProviderGroup, model: ModelProviderGroup['models'][number]): string {
 ...
 * （那会把高亮画到别的字符上，甚至越界）。
 */
export function dmsNameHit(name: string, normalizedQuery: string): { start: number; end: number } | null {
  if (normalizedQuery === '') return null
  const folded = dmsFold(name)
  if (folded.length !== name.length) return null
  const at = folded.indexOf(normalizedQuery)
  return at < 0 ? null : { start: at, end: at + normalizedQuery.length }
}
...
export function dmsSearchHits(
  ...
  for (const choice of choices) {
    ...
    items.push({ group: choice.group, model: choice.model, nameHit: dmsNameHit(choice.model.name, normalizedQuery) })
  }
  return { items, total }
}
 ...
 */
export function dmsMenuEmptyState(input: {
  status: string
  choiceCount: number
  searching: boolean
  hitCount: number
